...
PeerServer.prototype._initializeHTTP = function() {
  ...
  if (this._options["static"]) {
    console.log("doe",this._options.path+this._options.static);
    this._app.get(/\/static\/?.*/ , restify.serveStatic({
      directory : this._options.static.directory,
      default   : this._options.static.default
    }));
  }

  // additional routes defined at options.routes[].
  // eg.
  // ps = new PeerServer({
  //   routes:
  //   [
  ...
  //   ]
  // });
  // GET:localhost/hi will serve 'hello world'
  // note this will override ps.path

  if (this._options.routes) {
    (function(_this) {
      return (function() {
        var route, _i, _len, _ref, _results;
        _ref = _this._options.routes;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          route = _ref[_i];
          _results.push(_this._app[route.type](route.path, route.fn));
        }
        return _results;
      });
    })(this)();
